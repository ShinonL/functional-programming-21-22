...
compare (Interval intA) (Interval intB) =
    case (intA.end, intB.end) of
       (Just e1, Just e2) -> chainCompare (Date.compare e1 e2) (Date.compare intA.start intB.start)
       (Nothing, Nothing) -> Date.compare intA.start intB.start
       (Just _, Nothing) -> chainCompare GT <| Date.compare intA.start intB.start
       (Nothing, Just _) -> chainCompare LT <| Date.compare intA.start intB.start
{- Another solution for the first case:
     Date.compare e1 e2 |> chainCompare <| Date.compare intA.start intB.start
-}
...
view interval =
    let
        (Interval recordInterval) = interval
        displayEnd = 
            case recordInterval.end of
               Nothing -> text "Present"
               Just date -> Date.view date
        checkHidden = 
            case length interval of
               Just _ -> [ class "interval-length" ]
               Nothing -> [ class "interval-length", hidden True ]
        displayLength =
            case length interval of
                Just (years, months) -> String.fromInt months ++ " months " ++ 
                    case years of
                        0 -> ""
                        _ -> String.fromInt years ++ " years"
                Nothing -> ""
    in
    div [ class "interval" ] [
        span [ class "interval-start" ] [ 
            text "Event During: "
        ,   Date.view recordInterval.start ]
    ,   span [ class "interval-end" ] [ 
            text " - "
        ,   displayEnd 
        ]
    ,   div (checkHidden) [ text <| "Duration: " ++ displayLength ]
    ]
